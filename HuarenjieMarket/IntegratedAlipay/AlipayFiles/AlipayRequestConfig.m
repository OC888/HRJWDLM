...
+ (void)alipayWithPartner:(NSString *)partner
    ...
    NSLog(@"signSSSS   =   222%@222",signedString);

    if (signedString != nil) {
        
//        NSLog(@"-------orderSpec:%@ -----signedString:%@ -------NEXT%@",orderSpec,signedString,nextS);
        // 调用支付接口
...
+ (void)payWithAppScheme:(NSString *)appScheme orderSpec:(NSString *)orderSpec signedString:(NSString *)signedString {
    ...
    if (signedString != nil) {
        ...
        NSLog(@"SSSSSSSSSASFDSADFSDFSS   = %@",signedString);
