...
- (void)viewDidLoad
{
    ...
    UIButton *rightBtn = [[UIButton alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width - 44, 0, 44, 44)];
...
- (void)uploadAction:(UIButton *)sender {
    ...
    [manager POST:@"http://zghrj.cn/interface/shipin_upload.php" parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        [formData appendPartWithFileData:videoData name:@"file1" fileName:fileName mimeType:@"application/octet-stream"];
    } success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
        NSLog(@"------成功");
        UIAlertView *alertV = [[UIAlertView alloc] initWithTitle:nil message:@"上传成功" delegate:self cancelButtonTitle:nil otherButtonTitles:nil];
        ...
        UIAlertView *alertV = [[UIAlertView alloc] initWithTitle:nil message:@"未能成功上传" delegate:self cancelButtonTitle:nil otherButtonTitles:nil];
        [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(timeFireMethod:) userInfo:alertV repeats:NO];
        [alertV show];
    }];
}
