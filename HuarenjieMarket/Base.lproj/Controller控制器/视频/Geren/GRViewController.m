...
@interface GRViewController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate>
...
- (void)netWorking {
    ...
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"text/html", @"application/json", @"xml", @"script",  nil];
    __weak GRViewController *GerenVC = self;
    [manager GET:@"http://zghrj.cn/interface/shipin_select.php" parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        
        [GerenVC parseData:responseObject];
        NSLog(@"+++++++网络请求成功");
...
-(void)startPlayVideo:(UIButton *)sender{
    ...
    self.currentCell = (THVideoCell *)[self.table cellForRowAtIndexPath:_currentIndexPath];
    GerenModel *model = [_dataSource objectAtIndex:sender.tag];
    
    
    if (_htPlayer) {
        [_htPlayer removeFromSuperview];
        [_htPlayer setVideoURLStr:model.shipinurl];
        
    }else{
        _htPlayer = [[HTPlayer alloc]initWithFrame:self.currentCell.backView.bounds videoURLStr:model.shipinurl];
    }
    
    [_htPlayer setPlayTitle:model.uploaddatetime];
