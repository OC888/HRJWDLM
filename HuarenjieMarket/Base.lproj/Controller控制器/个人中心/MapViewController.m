...
- (void)lpgrClick:(UILongPressGestureRecognizer *)lpgr
{
    ...
    if(lpgr.state == UIGestureRecognizerStateBegan)
    {
        ...
        pinAnnotation.title = [NSString stringWithFormat:@"经度%.2f 纬度%.2f",center.latitude,center.longitude];
        [_mapView addAnnotation:pinAnnotation];
        UIBarButtonItem *barb = [[UIBarButtonItem alloc] initWithTitle:@"提交位置" style:UIBarButtonSystemItemAdd target:self action:@selector(actions1:)];
        self.navigationItem.rightBarButtonItem = barb;
        
        UIBarButtonItem *barb2 = [[UIBarButtonItem alloc] initWithTitle:@"移除位置" style:UIBarButtonSystemItemCancel target:self action:@selector(actions2:)];
        self.navigationItem.leftBarButtonItem = barb2;
    }
}
-(void)actions1:(UIBarButtonItem *)sender{
    //    提交用户位置
    //提交成功之后
    UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:@"成功" message:@"位置已提交" preferredStyle:UIAlertControllerStyleAlert];
...
-(void)actions2:(UIBarButtonItem *)sender {
    //    移除所有大头针
