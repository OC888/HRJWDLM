...
-(void)setState:(BOOL)state{
    if (state==YES) {
        self.labName.textColor=self.selectedColor?self.selectedColor:[UIColor blackColor];
        self.userInteractionEnabled=NO;
    }else if(state==NO)
    {
        self.labName.textColor=self.notSelectedColor?self.notSelectedColor:[UIColor lightGrayColor];
