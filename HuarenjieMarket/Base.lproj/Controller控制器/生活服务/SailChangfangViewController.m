...
@interface SailChangfangViewController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UITextFieldDelegate>
{
    NSArray *_addressArr;
    NSArray *_leixingArr;
...
@implementation SailChangfangViewController
...
-(void)doBack:(id)sender{
    ...
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        ...
        [[NSFileManager defaultManager] removeItemAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage2.plist"] error:nil];
    }];
...
-(void)resumeButtonClick{
    ...
    if ([UDF objectForKey:@"sailmianji2"]) {
        dec_mianji=[UDF objectForKey:@"sailmianji2"];
    }else{
        dec_mianji=@"";
    }
    if ([UDF objectForKey:@"sailhouseAddress2"]) {
        dizhi=[UDF objectForKey:@"sailhouseAddress2"];
    }else{
        dizhi=@"";
    }
    if ([UDF objectForKey:@"sailzujin2"]) {
        dec_price=[UDF objectForKey:@"sailzujin2"];
    }else{
        dec_price=@"";
    }
    if ([UDF objectForKey:@"sailgongxu2"]) {
        gongxu=[UDF objectForKey:@"sailgongxu2"];
    }else{
        gongxu=@"";
    }
    if ([UDF objectForKey:@"sailleixing2"]) {
        leixing=[UDF objectForKey:@"sailleixing2"];
    }else{
        leixing=@"";
    }
    if ([UDF objectForKey:@"sailperson2"]) {
        lianxiren=[UDF objectForKey:@"sailperson2"];
    }else{
        lianxiren=@"";
    }
    if ([UDF objectForKey:@"sailhouseDescribe2"]) {
        miaoshu=[UDF objectForKey:@"sailhouseDescribe2"];
    }else{
        miaoshu=@"";
    }
    if ([UDF objectForKey:@"sailphone2"]) {
        phone=[UDF objectForKey:@"sailphone2"];
    }else{
        phone=@"";
    }
    if ([UDF objectForKey:@"sailaddress2"]) {
        quyu=[UDF objectForKey:@"sailaddress2"];
    }else{
        quyu=@"";
    }
    if ([UDF objectForKey:@"sailhouseTitle2"]) {
        title=[UDF objectForKey:@"sailhouseTitle2"];
    }else{
        title=@"";
    }
    if ([UDF objectForKey:@"sailneede2"]) {
        xinxilaiyuan=[UDF objectForKey:@"sailneede2"];
    }else{
        xinxilaiyuan=@"";
    }
    
    NSArray *imageArr;
    if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage2.plist"]]) {
        imageArr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage2.plist"]];
    }
    NSData *data1;
    NSData *data2;
    NSData *data3;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *section3PlaceholderArr=@[@"6-30字，不能填写电话",@"地理位置，交通情况，等等"];
    ...
    if (indexPath.section==0) {
        ...
        for (UIButton *button in _imageButtonArr) {
            if (button.tag==1) {
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage2.plist"]]) {
                    NSArray *arr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage2.plist"]];
        ...
        if (indexPath.row==0) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailaddress2"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailhouseAddress2"];
        ...
        if (indexPath.row==0) {
            ...
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"sailgongxu2"] isEqualToString:@"求租"]) {
                seg.selectedSegmentIndex=1;
            }else if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"sailgongxu2"] isEqualToString:@"出售"]) {
                seg.selectedSegmentIndex=2;
            }else if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"sailgongxu2"] isEqualToString:@"求购"]) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailzujin2"];
            ...
            label.textAlignment=NSTextAlignmentRight;
            label.font=[UIFont systemFontOfSize:14.0];
            cell.detailTextField.text=[self writeWithKey:@"sailmianji2"];
            ...
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.text=[self writeWithKey:@"sailleixing2"];
        ...
        if (indexPath.row==0){
            cell.detailTextField.text=[self writeWithKey:@"sailhouseTitle2"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailhouseDescibe2"];
        ...
        if (indexPath.row==0) {
            cell.detailTextField.text=[self writeWithKey:@"sailperson2"];
            ...
            cell.detailTextField.keyboardType=UIKeyboardTypeNumberPad;
            cell.detailTextField.text=[self writeWithKey:@"sailphone2"];
            ...
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"sailneede2"] isEqualToString:@"经纪人"]) {
                seg.selectedSegmentIndex=1;
            }else{
                seg.selectedSegmentIndex=0;
            }
...
-(void)changeGongxu:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"出租" forKey:@"sailgongxu2"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"求租" forKey:@"sailgongxu2"];
        }
            break;
        case 2:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"出售" forKey:@"sailgongxu2"];
        }
            break;
        case 3:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"求购" forKey:@"sailgongxu2"];
        }
            break;
        default:
            break;
...
-(void)changeNeede5:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"个人" forKey:@"sailneede2"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"经纪人" forKey:@"sailneede2"];
        }
            break;
        default:
            break;
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [_imageArr writeToFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage2.plist"] atomically:YES];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==1) {
        if (indexPath.row==0) {
            ...
            ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择区域" textArray:_addressArr withKey:@"sailaddress2"];
    ...
    if (indexPath.section==2&&indexPath.row==3){
        ...
        ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"类型" textArray:_leixingArr withKey:@"sailleixing2"];
...
-(void)chooseAddressClick:(UIButton *)button{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"sailaddress2"];
...
-(void)chooseLeixingClick:(UIButton *)button{
    ...
    ResumeTableViewCell *cell=(ResumeTableViewCell *)[_tableView cellForRowAtIndexPath:indexPath];
    cell.detailTextField.text=text;
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"sailleixing2"];
