//
//  SailErShouHouseViewController.m
//  temp
//
...
@interface SailErShouHouseViewController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UITextFieldDelegate>
{
    NSArray *_addressArr;
    NSArray *_peizhiArr;
...
@implementation SailErShouHouseViewController
...
-(void)doBack:(id)sender{
    ...
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        ...
        [[NSFileManager defaultManager] removeItemAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage1.plist"] error:nil];
    }];
...
-(void)resumeButtonClick{
    ...
    if ([UDF objectForKey:@"sailchaoxiang1"]) {
        chaoxiang=[UDF objectForKey:@"sailchaoxiang1"];
    }else{
        chaoxiang=@"";
    }
    if ([UDF objectForKey:@"sailfangling1"]) {
        dec_fangling=[UDF objectForKey:@"sailfangling1"];
    }else{
        dec_fangling=@"";
    }
    if ([UDF objectForKey:@"sailmianji1"]) {
        dec_mianji=[UDF objectForKey:@"sailmianji1"];
    }else{
        dec_mianji=@"";
    }
    if ([UDF objectForKey:@"sailshoujia1"]) {
        dec_price=[UDF objectForKey:@"sailshoujia1"];
    }else{
        dec_price=@"";
    }
    if ([UDF objectForKey:@"sailhuxing1"]) {
        huxing=[UDF objectForKey:@"sailhuxing1"];
    }else{
        huxing=@"";
    }
    if ([UDF objectForKey:@"sailperson1"]) {
        lianxiren=[UDF objectForKey:@"sailperson1"];
    }else{
        lianxiren=@"";
    }
    if ([UDF objectForKey:@"saillouceng1"]) {
        louceng=[UDF objectForKey:@"saillouceng1"];
    }else{
        louceng=@"";
    }
    if ([UDF objectForKey:@"sailhouseDescribe1"]) {
        miaoshu=[UDF objectForKey:@"sailhouseDescribe1"];
    }else{
        miaoshu=@"";
    }
    if ([UDF objectForKey:@"sailphone1"]) {
        phone=[UDF objectForKey:@"sailphone1"];
    }else{
        phone=@"";
    }
    if ([UDF objectForKey:@"sailhouseName1"]) {
        quyu=[UDF objectForKey:@"sailhouseName1"];
    }else{
        quyu=@"";
    }
    if ([UDF objectForKey:@"sailhouseTitle1"]) {
        title=[UDF objectForKey:@"sailhouseTitle1"];
    }else{
        title=@"";
    }
    if ([UDF objectForKey:@"sailaddress1"]) {
        xiaoqu=[UDF objectForKey:@"sailaddress1"];
    }else{
        xiaoqu=@"";
    }
    if ([UDF objectForKey:@"sailneede1"]) {
        xinxilaiyuan=[UDF objectForKey:@"sailneede1"];
    }else{
        xinxilaiyuan=@"";
    }

    NSArray *imageArr;
    if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage1.plist"]]) {
        imageArr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage1.plist"]];
    }
    NSData *data1;
    NSData *data2;
    NSData *data3;
...
{
    if (textField.tag==6||textField.tag==7||textField.tag==2||textField.tag==8) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *section2ListArr=@[@"售       价",@"户       型",@"建筑面积",@"楼       层",@"朝       向",@"房       龄"];
    NSArray *section2PlaceholderArr=@[@"数字填写，不能是0",@"请选择户型",@"数字填写，请输入",@"请选择楼层",@"请选择",@"数字填写，最多一位小数"];
    ...
    if (indexPath.section==0) {
        ...
        for (UIButton *button in _imageButtonArr) {
            if (button.tag==1) {
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage1.plist"]]) {
                    NSArray *arr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage1.plist"]];
        ...
        if (indexPath.row==0) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailaddress1"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailhouseName1"];
        ...
        if (indexPath.row==0) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailshoujia1"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveShoujia1:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            cell.detailTextField.tag=7;
        }else if (indexPath.row==1) {
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.enabled=NO;
            cell.detailTextField.text=[self writeWithKey:@"sailhuxing1"];
            ...
            label.textAlignment=NSTextAlignmentRight;
            label.font=[UIFont systemFontOfSize:14.0];
            cell.detailTextField.text=[self writeWithKey:@"sailmianji1"];
            ...
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.text=[self writeWithKey:@"saillouceng1"];
            ...
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.text=[self writeWithKey:@"sailchaoxiang1"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailfangling1"];
        ...
        if (indexPath.row==0){
            cell.detailTextField.text=[self writeWithKey:@"sailhouseTitle1"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailhouseDescribe1"];
        ...
        if (indexPath.row==0) {
            cell.detailTextField.text=[self writeWithKey:@"sailperson1"];
            ...
            cell.detailTextField.keyboardType=UIKeyboardTypeNumberPad;
            cell.detailTextField.text=[self writeWithKey:@"sailphone1"];
            ...
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"sailneede1"] isEqualToString:@"经纪人"]) {
...
-(void)changeNeede:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"个人" forKey:@"sailneede1"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"经纪人" forKey:@"sailneede1"];
        }
            break;
        default:
            break;
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [_imageArr writeToFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage1.plist"] atomically:YES];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==1) {
        if (indexPath.row==0) {
            ...
            ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择区域" textArray:_addressArr withKey:@"sailaddress1"];
    ...
    if (indexPath.section==2&&indexPath.row==4) {
        ...
        ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择朝向" textArray:_peizhiArr withKey:@"sailchaoxinag1"];
...
-(void)chooseAddressClick:(UIButton *)button{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"sailaddress1"];
...
-(void)choosePeizhiClick:(UIButton *)button{
    ...
    ResumeTableViewCell *cell=(ResumeTableViewCell *)[_tableView cellForRowAtIndexPath:indexPath];
    cell.detailTextField.text=text;
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"sailchaoxiang1"];
...
-(void)chooseHuxing{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:_huxingPickerView.pickerString  forKey:@"sailhuxing1"];
    [self datePickerDown];
}
-(void)chooseLouceng{
    NSIndexPath *indexPath=[NSIndexPath indexPathForRow:3 inSection:2];
    ...
    [[NSUserDefaults standardUserDefaults]setObject:_loucengPickerView.pickerString  forKey:@"saillouceng1"];
    [self datePickerDown];
}
