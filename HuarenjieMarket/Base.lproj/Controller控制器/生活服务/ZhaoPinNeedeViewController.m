...
#import "ZhaoPinNeedeViewController.h"

@interface ZhaoPinNeedeViewController ()<UITableViewDelegate,UITableViewDataSource,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UITextViewDelegate,UITextFieldDelegate>
{
...
@implementation ZhaoPinNeedeViewController
...
-(void)resumeButtonClick{
    ...
    ZhaoPinCompanyInfoViewController *vc =[[ZhaoPinCompanyInfoViewController alloc]init];
    vc.area = area;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *section1ListArr=@[@"工作年限",@"月       薪",@"学       历"];
    NSArray *section1PlaceholderArr=@[@"不限",@"面议",@"不限"];
    NSArray *section2ListArr=@[@"招聘人数",@"年龄段"];
    NSArray *section2PlaceholderArr=@[@"数字填写，不能是0和负数",@"不限"];
    NSArray *section3ListArr=@[@"区       域",@"工作地点"];
    NSArray *section3PlaceholderArr=@[@"请选择",@"7-40字的详细地址"];
...
-(void)saveWorkDescribe:(NSNotification *)not{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:textView.text  forKey:@"workdescribe"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==1) {
        if (indexPath.row==0) {
            ...
            ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"月      薪" textArray:_moneyArr withKey:@"workmoney"];
            [vc.choosButton addTarget:self action:@selector(chooseWorkMoneyClick:) forControlEvents:UIControlEventTouchUpInside];
            [self.navigationController pushViewController:vc animated:YES];
...
//选择薪资
-(void)chooseWorkMoneyClick:(UIButton *)button{
    NSString *text=_moneyArr[button.tag-1];
    NSIndexPath *indexPath=[NSIndexPath indexPathForRow:1 inSection:1];
    ResumeTableViewCell *cell=(ResumeTableViewCell *)[_tableView cellForRowAtIndexPath:indexPath];
    cell.detailTextField.text=text;
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"workmoney"];
    [self.navigationController popViewControllerAnimated:YES];
}
//创建选择器
