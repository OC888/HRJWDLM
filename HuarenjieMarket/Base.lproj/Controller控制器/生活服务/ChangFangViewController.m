...
@interface ChangFangViewController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UITextFieldDelegate>
{
    NSArray *_addressArr;
    NSArray *_leixingArr;
...
@implementation ChangFangViewController
...
-(void)doBack:(id)sender{
    ...
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        ...
        [[NSFileManager defaultManager] removeItemAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage7.plist"] error:nil];
    }];
...
-(void)resumeButtonClick{
    ...
    if ([UDF objectForKey:@"houseTitle7"]) {
        biaoti=[UDF objectForKey:@"houseTitle7"];
    }else{
        biaoti=@"";
    }
    if ([UDF objectForKey:@"zujin7"]) {
        dec_zujin=[UDF objectForKey:@"zujin7"];
    }else{
        dec_zujin=@"";
    }
    if ([UDF objectForKey:@"houseAddress7"]) {
        dizhi=[UDF objectForKey:@"houseAddress7"];
    }else{
        dizhi=@"";
    }
    if ([UDF objectForKey:@"gongxu7"]) {
        gongxu=[UDF objectForKey:@"gongxu7"];
    }else{
        gongxu=@"";
    }
    if ([UDF objectForKey:@"leixing7"]) {
        leixing=[UDF objectForKey:@"leixing7"];
    }else{
        leixing=@"";
    }
    if ([UDF objectForKey:@"person7"]) {
        lianxiren=[UDF objectForKey:@"person7"];
    }else{
        lianxiren=@"";
    }
    if ([UDF objectForKey:@"houseDescribe7"]) {
        miaoshu=[UDF objectForKey:@"houseDescribe7"];
    }else{
        miaoshu=@"";
    }
    if ([UDF objectForKey:@"phone7"]) {
        phone=[UDF objectForKey:@"phone7"];
    }else{
        phone=@"";
    }
    if ([UDF objectForKey:@"address7"]) {
        quyu=[UDF objectForKey:@"address7"];
    }else{
        quyu=@"";
    }
    if ([UDF objectForKey:@"neede7"]) {
        xinxilaiyuan=[UDF objectForKey:@"neede7"];
    }else{
        xinxilaiyuan=@"";
    }
    if ([UDF objectForKey:@"mianji7"]) {
        dec_mianji=[UDF objectForKey:@"mianji7"];
    }else{
        dec_mianji=@"";
    }
    
    NSArray *imageArr;
    if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage7.plist"]]) {
        imageArr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage7.plist"]];
    }
    NSData *data1;
    NSData *data2;
    NSData *data3;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *section2ListArr=@[@"供       需",@"租       金",@"面       积",@"类       型"];
    ...
    if (indexPath.section==0) {
        ...
        for (UIButton *button in _imageButtonArr) {
            if (button.tag==1) {
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage7.plist"]]) {
                    NSArray *arr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage7.plist"]];
        ...
        if (indexPath.row==0) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"address7"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"houseAddress7"];
        ...
        if (indexPath.row==0) {
            UISegmentedControl *seg =[[UISegmentedControl alloc]initWithItems:@[@"出租",@"求租",@"出售",@"求购"]];
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"gongxu7"] isEqualToString:@"求租"]) {
                seg.selectedSegmentIndex=1;
            }else if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"gongxu7"] isEqualToString:@"出售"]) {
                seg.selectedSegmentIndex=2;
            }else if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"gongxu7"] isEqualToString:@"求购"]) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"zujin7"];
            ...
            label.textAlignment=NSTextAlignmentRight;
            label.font=[UIFont systemFontOfSize:14.0];
            cell.detailTextField.text=[self writeWithKey:@"mianji7"];
            ...
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.text=[self writeWithKey:@"leixing7"];
        ...
        if (indexPath.row==0){
            cell.detailTextField.text=[self writeWithKey:@"houseTitle7"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"houseDescribe7"];
        ...
        if (indexPath.row==0) {
            cell.detailTextField.text=[self writeWithKey:@"person7"];
            ...
            cell.detailTextField.keyboardType=UIKeyboardTypeNumberPad;
            cell.detailTextField.text=[self writeWithKey:@"phone7"];
            ...
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"neede7"] isEqualToString:@"经纪人"]) {
                seg.selectedSegmentIndex=1;
            }else{
                seg.selectedSegmentIndex=0;
            }
...
-(void)changeGongxu:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"出租" forKey:@"gongxu7"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"求组" forKey:@"gongxu7"];
        }
            break;
        case 2:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"出售" forKey:@"gongxu7"];
        }
            break;
        case 3:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"求购" forKey:@"gongxu7"];
        }
            break;
        default:
            break;
...
-(void)changeNeede5:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"个人" forKey:@"neede7"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"经纪人" forKey:@"neede7"];
        }
            break;
        default:
            break;
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [_imageArr writeToFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage7.plist"] atomically:YES];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==1) {
        if (indexPath.row==0) {
            ...
            ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择区域" textArray:_addressArr withKey:@"address7"];
    ...
    if (indexPath.section==2&&indexPath.row==3){
        ...
        _leixingArr=@[@"厂房",@"仓库",@"车位",@"土地",@"其他"];
        ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"类型" textArray:_leixingArr withKey:@"leixing7"];
...
-(void)chooseAddressClick:(UIButton *)button{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"address7"];
...
-(void)chooseLeixingClick:(UIButton *)button{
    ...
    ResumeTableViewCell *cell=(ResumeTableViewCell *)[_tableView cellForRowAtIndexPath:indexPath];
    cell.detailTextField.text=text;
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"leixing7"];
