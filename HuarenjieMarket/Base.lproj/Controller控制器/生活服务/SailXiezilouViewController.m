...
@interface SailXiezilouViewController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UITextFieldDelegate>
{
    NSArray *_addressArr;
    NSArray *_leixingArr;
...
@implementation SailXiezilouViewController
...
-(void)doBack:(id)sender{
    ...
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        ...
        [[NSFileManager defaultManager] removeItemAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage4.plist"] error:nil];
    }];
...
-(void)resumeButtonClick{
    ...
    if ([UDF objectForKey:@"sailmianji4"]) {
        dec_mianji=[UDF objectForKey:@"sailmianji4"];
    }else{
        dec_mianji=@"";
    }
    if ([UDF objectForKey:@"sailhouseAddress4"]) {
        dizhi=[UDF objectForKey:@"sailhouseAddress4"];
    }else{
        dizhi=@"";
    }
    if ([UDF objectForKey:@"sailzujin4"]) {
        dec_price=[UDF objectForKey:@"sailzujin4"];
    }else{
        dec_price=@"";
    }
    if ([UDF objectForKey:@"sailgongxu4"]) {
        gongxu=[UDF objectForKey:@"sailgongxu4"];
    }else{
        gongxu=@"";
    }
    if ([UDF objectForKey:@"sailleixing4"]) {
        leixing=[UDF objectForKey:@"sailleixing4"];
    }else{
        leixing=@"";
    }
    if ([UDF objectForKey:@"sailperson4"]) {
        lianxiren=[UDF objectForKey:@"sailperson4"];
    }else{
        lianxiren=@"";
    }
    if ([UDF objectForKey:@"sailhouseDescribe4"]) {
        miaoshu=[UDF objectForKey:@"sailhouseDescribe4"];
    }else{
        miaoshu=@"";
    }
    if ([UDF objectForKey:@"sailphone4"]) {
        phone=[UDF objectForKey:@"sailphone4"];
    }else{
        phone=@"";
    }
    if ([UDF objectForKey:@"sailaddress4"]) {
        quyu=[UDF objectForKey:@"sailaddress4"];
    }else{
        quyu=@"";
    }
    if ([UDF objectForKey:@"sailhouseTitle4"]) {
        title=[UDF objectForKey:@"sailhouseTitle4"];
    }else{
        title=@"";
    }
    if ([UDF objectForKey:@"sailneede4"]) {
        xinxilaiyuan=[UDF objectForKey:@"sailneede4"];
    }else{
        xinxilaiyuan=@"";
    }
    
    NSArray *imageArr;
    if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage4.plist"]]) {
        imageArr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage4.plist"]];
    }
    NSData *data1;
    NSData *data2;
    NSData *data3;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *section2ListArr=@[@"供       需",@"售       价",@"商铺名称",@"面       积",@"类       型"];
    ...
    if (indexPath.section==0) {
        ...
        for (UIButton *button in _imageButtonArr) {
            if (button.tag==1) {
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage4.plist"]]) {
                    NSArray *arr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage4.plist"]];
        ...
        if (indexPath.row==0) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailaddress4"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailhouseAddress4"];
        ...
        if (indexPath.row==0) {
            ...
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"sailgongxu4"] isEqualToString:@"求购"]) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailzujin4"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailname4"];
            ...
            label.textAlignment=NSTextAlignmentRight;
            label.font=[UIFont systemFontOfSize:14.0];
            cell.detailTextField.text=[self writeWithKey:@"sailmianji4"];
            ...
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.text=[self writeWithKey:@"sailleixing4"];
        ...
        if (indexPath.row==0){
            cell.detailTextField.text=[self writeWithKey:@"sailhouseTitle4"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"sailhouseDescibe4"];
        ...
        if (indexPath.row==0) {
            cell.detailTextField.text=[self writeWithKey:@"sailperson4"];
            ...
            cell.detailTextField.keyboardType=UIKeyboardTypeNumberPad;
            cell.detailTextField.text=[self writeWithKey:@"sailphone4"];
            ...
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"sailneede4"] isEqualToString:@"经纪人"]) {
                seg.selectedSegmentIndex=1;
            }else{
                seg.selectedSegmentIndex=0;
            }
...
-(void)changeGongxu:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"出售" forKey:@"sailgongxu4"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"求购" forKey:@"sailgongxu4"];
        }
            break;
        default:
            break;
...
-(void)changeNeede5:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"个人" forKey:@"sailneede4"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"经纪人" forKey:@"sailneede4"];
        }
            break;
        default:
            break;
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [_imageArr writeToFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sailhouseImage4.plist"] atomically:YES];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==1) {
        if (indexPath.row==0) {
            ...
            ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择区域" textArray:_addressArr withKey:@"sailaddress4"];
    ...
    if (indexPath.section==2&&indexPath.row==4){
        ...
        ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择类型" textArray:_leixingArr withKey:@"sailleixing4"];
...
-(void)chooseAddressClick:(UIButton *)button{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"sailaddress4"];
...
-(void)chooseLeixingClick:(UIButton *)button{
    ...
    ResumeTableViewCell *cell=(ResumeTableViewCell *)[_tableView cellForRowAtIndexPath:indexPath];
    cell.detailTextField.text=text;
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"sailleixing4"];
