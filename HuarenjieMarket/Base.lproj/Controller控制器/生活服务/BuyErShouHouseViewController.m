//
//  BuyErShouHouseViewController.m
//  temp
//
...
@interface BuyErShouHouseViewController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UITextFieldDelegate>
{
    NSArray *_addressArr;
    NSArray *_zujinArr;
...
@implementation BuyErShouHouseViewController
...
-(void)resumeButtonClick{
    ...
    NSString *dec_qiwangprice;
    NSString *huxing;
    NSString *lianxiren;
    NSString *miaoshu;
    NSString *phone;
    NSString *qiwangxiaoqu;
    NSString *quyu;
    NSString *title;
    NSString *xinxilaiyuan;
    
    if ([UDF objectForKey:@"buyzujin4"]) {
        dec_qiwangprice=[UDF objectForKey:@"buyzujin4"];
    }else{
        dec_qiwangprice=@"";
    }
    if ([UDF objectForKey:@"buyhuxing4"]) {
        huxing=[UDF objectForKey:@"buyhuxing4"];
    }else{
        huxing=@"";
    }
    if ([UDF objectForKey:@"buyperson4"]) {
        lianxiren=[UDF objectForKey:@"buyperson4"];
    }else{
        lianxiren=@"";
    }
    if ([UDF objectForKey:@"buyhouseDescibe4"]) {
        miaoshu=[UDF objectForKey:@"buyhouseDescibe4"];
    }else{
        miaoshu=@"";
    }
    if ([UDF objectForKey:@"buyphonenum4"]) {
        phone=[UDF objectForKey:@"buyphonenum4"];
    }else{
        phone=@"";
    }
    if ([UDF objectForKey:@"buyhouseName4"]) {
        qiwangxiaoqu=[UDF objectForKey:@"buyhouseName4"];
    }else{
        qiwangxiaoqu=@"";
    }
    if ([UDF objectForKey:@"buyaddress4"]) {
        quyu=[UDF objectForKey:@"buyaddress4"];
    }else{
        quyu=@"";
    }
    if ([UDF objectForKey:@"buyhouseTitle4"]) {
        title=[UDF objectForKey:@"buyhouseTitle4"];
    }else{
        title=@"";
    }
    if ([UDF objectForKey:@"buyneede4"]) {
        xinxilaiyuan=[UDF objectForKey:@"buyneede4"];
    }else{
        xinxilaiyuan=@"";
    }
    ...
    if (_judgeArr.count==0) {
        if ([CheckText nameWithObject:lianxiren]==YES&&[CheckText phoneWithObject:phone]==YES) {
            [AFHttpManager sendErShouFangQiuGouWithdec_qiwangprice:dec_qiwangprice withdt_datetime:fabutime withhuxing:huxing withlianxiren:lianxiren withmiaoshu:miaoshu withphone:phone withqiwangxiaoqu:qiwangxiaoqu withquyu:quyu withtitle:title withxinxilaiyuan:xinxilaiyuan withType:^(NSString *string) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *section0ListArr=@[@"选择区域",@"期望小区"];
    NSArray *section0PlaceholderArr=@[@"请选择",@"2-20字，不能填写手机号"];
    NSArray *section1ListArr=@[@"户       型",@"期望售价"];
    ...
    if (indexPath.section==0) {
        ...
        if (indexPath.row==0) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"buyaddress4"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"buyhouseName4"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveHouseName4:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
        ...
        if (indexPath.row==0) {
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.enabled=NO;
            cell.detailTextField.text=[self writeWithKey:@"buyhuxing4"];
            ...
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.text=[self writeWithKey:@"buyzujin4"];
        ...
        if (indexPath.row==0){
            cell.detailTextField.text=[self writeWithKey:@"buyhouseTitle4"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"buyhouseDescibe4"];
        ...
        if (indexPath.row==0) {
            cell.detailTextField.text=[self writeWithKey:@"buyperson4"];
            ...
            cell.detailTextField.keyboardType=UIKeyboardTypeNumberPad;
            cell.detailTextField.text=[self writeWithKey:@"buyphonenum4"];
            ...
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"buyneede4"] isEqualToString:@"经纪人"]) {
...
-(void)changeNeede:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"个人" forKey:@"buyneede4"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"经纪人" forKey:@"buyneede4"];
        }
            break;
        default:
            break;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==1&&indexPath.row==1) {
        _zujinArr=@[@"30万以下",@"30-60万",@"60-100万",@"100-150万",@"150-200万",@"200万以上"];
        ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"期望售价" textArray:_zujinArr withKey:@"buyzujin4"];
...
-(void)chooseAddressClick:(UIButton *)button{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"buyaddress4"];
...
-(void)choosePeizhiClick:(UIButton *)button{
    ...
    ResumeTableViewCell *cell=(ResumeTableViewCell *)[_tableView cellForRowAtIndexPath:indexPath];
    cell.detailTextField.text=text;
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"buyzujin4"];
...
-(void)chooseHuxing{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:_huxingPickerView.pickerString  forKey:@"buyhuxing4"];
    [self datePickerDown];
}
//选择器下降
-(void)datePickerDown{
