...
@interface HeZuViewController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UITextFieldDelegate>
{
    NSArray *_addressArr;
    NSArray *_peizhiArr;
    NSArray *_leixingArr;
...
@implementation HeZuViewController
...
-(void)doBack:(id)sender{
    ...
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        ...
        [[NSFileManager defaultManager] removeItemAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage2.plist"] error:nil];
    }];
...
-(void)resumeButtonClick{
    ...
    if ([UDF objectForKey:@"address2"]) {
        quyu=[UDF objectForKey:@"address2"];
    }else{
        quyu=@"";
    }
    if ([UDF objectForKey:@"houseName2"]) {
        xiaoquname=[UDF objectForKey:@"houseName2"];
    }else{
        xiaoquname=@"";
    }
    if ([UDF objectForKey:@"huxing2"]) {
        huxing=[UDF objectForKey:@"huxing2"];
    }else{
        huxing=@"";
    }
    if ([UDF objectForKey:@"mianji2"]) {
        dec_mianji=[UDF objectForKey:@"mianji2"];
    }else{
        dec_mianji=@"";
    }
    if ([UDF objectForKey:@"louceng2"]) {
        louceng=[UDF objectForKey:@"louceng2"];
    }else{
        louceng=@"";
    }
    if ([UDF objectForKey:@"zujin2"]) {
        dec_zujin=[UDF objectForKey:@"zujin2"];
    }else{
        dec_zujin=@"";
    }
    if ([UDF objectForKey:@"peizhi2"]) {
        peizhi=[UDF objectForKey:@"peizhi2"];
    }else{
        peizhi=@"";
    }
    if ([UDF objectForKey:@"houseTitle2"]) {
        biaoti=[UDF objectForKey:@"houseTitle2"];
    }else{
        biaoti=@"";
    }
    if ([UDF objectForKey:@"houseDescribe2"]) {
        miaoshu=[UDF objectForKey:@"houseDescribe2"];
    }else{
        miaoshu=@"";
    }
    if ([UDF objectForKey:@"person2"]) {
        lianxiren=[UDF objectForKey:@"person2"];
    }else{
        lianxiren=@"";
    }
    if ([UDF objectForKey:@"phone22"]) {
        phone=[UDF objectForKey:@"phone22"];
    }else{
        phone=@"";
    }
    if ([UDF objectForKey:@"neede2"]) {
        xinxilaiyuan=[UDF objectForKey:@"neede2"];
    }else{
        xinxilaiyuan=@"";
    }
    if ([UDF objectForKey:@"leixing2"]) {
        hezuleixing=[UDF objectForKey:@"leixing2"];
    }else{
        hezuleixing=@"";
    }

    NSArray *imageArr;
    if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage2.plist"]]) {
        imageArr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage2.plist"]];
    }
    NSData *data1;
    NSData *data2;
    NSData *data3;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *section2ListArr=@[@"户       型",@"面       积",@"楼       层",@"租       金",@"合租类型"];
    NSArray *section2PlaceholderArr=@[@"请选择户型",@"数字填写，请输入",@"请选择楼层",@"数组填写，不能是0",@"请选择"];
    ...
    if (indexPath.section==0) {
        ...
        for (UIButton *button in _imageButtonArr) {
            if (button.tag==1) {
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage2.plist"]]) {
                    NSArray *arr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage2.plist"]];
        ...
        if (indexPath.row==0) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"address2"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveAddress2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
        }else {
            cell.detailTextField.text=[self writeWithKey:@"houseName2"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveHouseName2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
        ...
        if (indexPath.row==0) {
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.enabled=NO;
            cell.detailTextField.text=[self writeWithKey:@"huxing2"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveHuXing2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            ...
            cell.detailTextField.text=[self writeWithKey:@"mianji2"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveMianji2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            cell.detailTextField.tag=8;
        }else if (indexPath.row==2){
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.text=[self writeWithKey:@"louceng2"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveLouceng2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            cell.detailTextField.enabled=NO;
            ...
            label.textAlignment=NSTextAlignmentRight;
            label.font=[UIFont systemFontOfSize:14.0];
            cell.detailTextField.text=[self writeWithKey:@"zujin2"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveZujin2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            cell.detailTextField.tag=7;
        }else{
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.text=[self writeWithKey:@"leixing2"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveLeixing2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            cell.detailTextField.enabled=NO;
        }
        ...
        if (indexPath.row==0) {
            cell.detailTextField.text=[self writeWithKey:@"peizhi2"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(savePeizhi2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            cell.detailTextField.enabled=NO;
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
        }else if (indexPath.row==1){
            cell.detailTextField.text=[self writeWithKey:@"houseTitle2"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveHouseTitle2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            ...
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveHouseDescibe2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            cell.detailTextField.text=[self writeWithKey:@"houseDescribe2"];
        ...
        if (indexPath.row==0) {
            cell.detailTextField.text=[self writeWithKey:@"person2"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(savePerson2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            cell.detailTextField.tag=1;
        }else if (indexPath.row==1){
            cell.detailTextField.keyboardType=UIKeyboardTypeNumberPad;
            cell.detailTextField.text=[self writeWithKey:@"phone22"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(savePhone2:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            cell.detailTextField.tag=2;
        }else{
            UISegmentedControl *seg =[[UISegmentedControl alloc]initWithItems:@[@"个人",@"经纪人"]];
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"neede2"] isEqualToString:@"经纪人"]) {
...
-(void)changeNeede:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"个人" forKey:@"neede2"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"经纪人" forKey:@"neede2"];
        }
            break;
        default:
            break;
...
-(void)saveAddress2:(NSNotification *)not{
    [self save:@"address2" withNot:not];
    NSIndexPath *indexPath =[NSIndexPath indexPathForRow:0 inSection:1];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
}
-(void)saveHouseName2:(NSNotification *)not{
    [self save:@"houseName2" withNot:not];
    NSIndexPath *indexPath =[NSIndexPath indexPathForRow:1 inSection:1];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
}
-(void)saveHuXing2:(NSNotification *)not{
    [self save:@"huxing2" withNot:not];
    NSIndexPath *indexPath =[NSIndexPath indexPathForRow:0 inSection:2];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
}
-(void)saveMianji2:(NSNotification *)not{
    [self save:@"mianji2" withNot:not];
    NSIndexPath *indexPath =[NSIndexPath indexPathForRow:1 inSection:2];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
}
-(void)saveLouceng2:(NSNotification *)not{
    [self save:@"louceng2" withNot:not];
    NSIndexPath *indexPath =[NSIndexPath indexPathForRow:2 inSection:2];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
}
-(void)saveZujin2:(NSNotification *)not{
    [self save:@"zujin2" withNot:not];
    NSIndexPath *indexPath =[NSIndexPath indexPathForRow:3 inSection:2];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
}
-(void)saveLeixing2:(NSNotification *)not{
    [self save:@"leixing2" withNot:not];
    NSIndexPath *indexPath =[NSIndexPath indexPathForRow:4 inSection:2];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
}
-(void)savePeizhi2:(NSNotification *)not{
    [self save:@"peizhi2" withNot:not];
    NSIndexPath *indexPath =[NSIndexPath indexPathForRow:0 inSection:3];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
}
-(void)saveHouseTitle2:(NSNotification *)not{
    [self save:@"houseTitle2" withNot:not];
    NSIndexPath *indexPath =[NSIndexPath indexPathForRow:1 inSection:3];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
}
-(void)saveHouseDescibe2:(NSNotification *)not{
    [self save:@"houseDescribe2" withNot:not];
    NSIndexPath *indexPath =[NSIndexPath indexPathForRow:2 inSection:3];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
}
-(void)savePerson2:(NSNotification *)not{
    [self save:@"person2" withNot:not];
    NSIndexPath *indexPath =[NSIndexPath indexPathForRow:0 inSection:4];
    [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
}
-(void)savePhone2:(NSNotification *)not{
    [self save:@"phone22" withNot:not];
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [_imageArr writeToFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage2.plist"] atomically:YES];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==1) {
        if (indexPath.row==0) {
            ...
            ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择区域" textArray:_addressArr withKey:@"address2"];
    ...
    if (indexPath.section==3&&indexPath.row==0) {
        ...
        ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择配置" textArray:_peizhiArr withKey:@"peizhi2"];
        [vc.choosButton addTarget:self action:@selector(choosePeizhiClick:) forControlEvents:UIControlEventTouchUpInside];
        [self.navigationController pushViewController:vc animated:YES];
    }
    if (indexPath.section==2&&indexPath.row==4) {
        _leixingArr=@[@"主卧",@"次卧",@"隔断间",@"合租床位"];
        ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择配置" textArray:_leixingArr withKey:@"leixing2"];
...
-(void)chooseAddressClick:(UIButton *)button{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"address2"];
...
-(void)choosePeizhiClick:(UIButton *)button{
    ...
    ResumeTableViewCell *cell=(ResumeTableViewCell *)[_tableView cellForRowAtIndexPath:indexPath];
    cell.detailTextField.text=text;
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"peizhi2"];
...
-(void)chooseLeixingClick:(UIButton *)button{
    ...
    ResumeTableViewCell *cell=(ResumeTableViewCell *)[_tableView cellForRowAtIndexPath:indexPath];
    cell.detailTextField.text=text;
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"leixing2"];
...
-(void)chooseHuxing{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:_huxingPickerView.pickerString  forKey:@"huxing2"];
    [self datePickerDown];
}
-(void)chooseLouceng{
    NSIndexPath *indexPath=[NSIndexPath indexPathForRow:2 inSection:2];
    ...
    [[NSUserDefaults standardUserDefaults]setObject:_loucengPickerView.pickerString  forKey:@"louceng2"];
    [self datePickerDown];
}
