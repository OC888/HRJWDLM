...
@interface PartNeedeViewController ()<UITableViewDelegate,UITableViewDataSource,UIActionSheetDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UITextFieldDelegate>
{
    UIView *_view;
    NSArray *_hangyeArr;
    NSArray *_guimoArr;
...
@implementation PartNeedeViewController
...
-(void)resumeButtonClick{
    ...
    if (_judgeArr.count==0) {
        if ([CheckText nameWithObject:_lianxiren]==YES&&[CheckText phoneWithObject:_phone]==YES) {
            [AFHttpManager sendPartTimeWitharea:self.area withdec_xinzi:self.dec_yuexin withdt_datetime:self.dt_datetime withgongsijianjie:gongsijianjie withgongsiname:gongsiname withgongzuodidian:self.gongzuodidian withgongzuonianxian:self.gongzuonianxian withgongzuoshijian:self.gongzuoshijian withguimo:guimo withi_zhaopinrenshu:_i_zhaopinrenshu withlianxiren:self.lianxiren withnianlingduan:self.nianlingduan withphone:self.phone withqitafuli:qitamiaoshu withshisufuli:shisufuli withshehuifuli:shehuifuli withtitle:self.jobtitle withxueli:self.xueli withzhiwei:self.zhiwei withzhiweimiaoshu:self.zhiweimiaoshu withType:^(NSString *string) {
