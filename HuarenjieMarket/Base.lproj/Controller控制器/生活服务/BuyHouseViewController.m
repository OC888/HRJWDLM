...
-(void)creatNavigationItem{
    ...
    self.search.searchBar.placeholder=@"搜索学区房";
...
//导航右栏按钮绑定方法
-(void)writeRsume{
    SailHouseViewController *vc =[[SailHouseViewController alloc]init];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!cell) {
        ...
        if (indexPath.section==0) {
            NSArray *imageArr=@[@"house_secondhouse",@"house_shangpuchushou",@"house_xiezilou"];
            _titleArr=@[@"二手房",@"商铺出售",@"写字楼出售"];
            for (int i=0; i<3; i++) {
            ...
            if (indexPath.row==0) {
                ...
                NSArray *imageArr=@[@"fangjia",@"xuequ"];
                for (int i=0; i<2; i++) {
                    UIButton *button =[UIButton buttonWithType:UIButtonTypeCustom];
                    int col =i%2;
                    int row =i/2;
                ...
                NSArray *arr=@[@"二手房",@"二手房求购",@"精装修",@"低首付",@"小区找房",@"特价新房"];
                for (int i=0; i<6; i++) {
                    int col =i%3;
                    int row =i/3;
                    CarButton *button =[[CarButton alloc]initWithFrame:CGRectMake(col*WIDTH/3,10+((SECTION1_HEIGHT-10*3)/2+10)*row, WIDTH/3, (SECTION1_HEIGHT-10*3)/2)];
            ...
            NSArray *arr=@[@"商铺出售",@"写字楼出售",@"厂房仓库土地"];
            [cell addSubview:[self creatViewWithHeight:SECTION2_HEIGHT withNumber:3 withRowNumber:3  withNsstring:@"商业地产" withImage:@"house_hezu" withArr:arr withTag:200]];
        }else if (indexPath.section==3) {
            cell.textLabel.text=@"房产资讯";
            cell.textLabel.font=[UIFont systemFontOfSize:15.0];
            cell.detailTextLabel.text=@"今天花百万当房奴是一种投资？";
            ...
            if (indexPath.row==0) {
                UIImageView *imageView =[[UIImageView alloc]initWithFrame:CGRectMake(10, 10, 20, 20)];
                imageView.image=[UIImage imageNamed:@"house_xiezilou"];
                [cell addSubview:imageView];
                UILabel *label =[[UILabel alloc]initWithFrame:CGRectMake(40, 10, 200, 20)];
                label.font=[UIFont systemFontOfSize:15.0];
                label.text=@"附近好房";
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==4) {
        if (indexPath.row==6) {
            RentHouseListViewController *vc=[[RentHouseListViewController alloc]init];
            [self.navigationController pushViewController:vc animated:YES];
        }else{
            SHOW_MESSAGE(@"数据加载中");
        }
