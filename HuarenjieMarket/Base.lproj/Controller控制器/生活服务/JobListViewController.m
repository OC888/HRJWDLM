...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [AFHttpManager parserFullTimeWithTitle:_placeholder withArr:^(NSMutableArray *resumeArr) {
        [mb hide:YES];
...
-(void)creatSendButton{
    ...
    _sendButton.frame=CGRectMake(WIDTH/4*3, HEIGHT/5*4, WIDTH/5, WIDTH/5);
    [_sendButton setBackgroundImage:[UIImage imageNamed:@"jianli-01"] forState:UIControlStateNormal];
    [_sendButton addTarget:self action:@selector(sendResum:) forControlEvents:UIControlEventTouchUpInside];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.checkButton.isSelect=NO;
    cell.checkButton.tag=indexPath.row;
    [cell.checkButton addTarget:self action:@selector(checkButtonClcik:) forControlEvents:UIControlEventTouchUpInside];
    [cell.checkButton setBackgroundImage:[UIImage imageNamed:@"widget_check_e"] forState:UIControlStateNormal];

    for (NSIndexPath *indexpath in _indexPathArr) {
        if (indexpath==indexPath) {
            cell.checkButton.isSelect=YES;
            [cell.checkButton setBackgroundImage:[UIImage imageNamed:@"widget_check_s"] forState:UIControlStateNormal];
        }
    }
    FullTimeInfo *info =_arr[indexPath.row];
    cell.titleLabel.text=info.title;
    cell.moneyLabel.text=info.dec_yuexin;
    ...
    cell.hotImageView.image=[UIImage imageNamed:@"gremen-01.png"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    vc.fullTimeInfo=_arr[indexPath.row];
    vc.tableViewtagStr=[NSString stringWithFormat:@"%ld",(long)self.tableView.tag];
    [self.navigationController pushViewController:vc animated:YES];
}
-(void)checkButtonClcik:(BoolButton *)button{

    if (button.isSelect==NO) {
        [button setBackgroundImage:[UIImage imageNamed:@"widget_check_s"] forState:UIControlStateNormal];
        [_buttonArr addObject:button];
        NSIndexPath *indexPath =[NSIndexPath indexPathForRow:button.tag inSection:0];
        [_indexPathArr addObject:indexPath];
        button.isSelect=YES;
    }else{
        [button setBackgroundImage:[UIImage imageNamed:@"widget_check_e"] forState:UIControlStateNormal];
        [_buttonArr removeObject:button];
        button.isSelect=NO;
        NSIndexPath *indexPath =[NSIndexPath indexPathForRow:button.tag inSection:0];
        [_indexPathArr removeObject:indexPath];
    }
    if (_indexPathArr.count>0) {
