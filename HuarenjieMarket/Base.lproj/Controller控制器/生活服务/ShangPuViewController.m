...
@interface ShangPuViewController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UITextFieldDelegate>
{
    NSArray *_addressArr;
    NSArray *_leixingArr;
...
-(void)doBack:(id)sender{
    ...
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        ...
        [[NSFileManager defaultManager] removeItemAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage5.plist"] error:nil];
    }];
...
-(void)resumeButtonClick{
    ...
    if ([UDF objectForKey:@"houseTitle5"]) {
        biaoti=[UDF objectForKey:@"houseTitle5"];
    }else{
        biaoti=@"";
    }
    if ([UDF objectForKey:@"zujin5"]) {
        dec_zujin=[UDF objectForKey:@"zujin5"];
    }else{
        dec_zujin=@"";
    }
    if ([UDF objectForKey:@"houseAddress5"]) {
        dizhi=[UDF objectForKey:@"houseAddress5"];
    }else{
        dizhi=@"";
    }
    if ([UDF objectForKey:@"gongxu5"]) {
        gongxu=[UDF objectForKey:@"gongxu5"];
    }else{
        gongxu=@"";
    }
    if ([UDF objectForKey:@"leixing5"]) {
        shangpuleixing=[UDF objectForKey:@"leixing5"];
    }else{
        shangpuleixing=@"";
    }
    if ([UDF objectForKey:@"person5"]) {
        lianxiren=[UDF objectForKey:@"person5"];
    }else{
        lianxiren=@"";
    }
    if ([UDF objectForKey:@"houseDescribe5"]) {
        miaoshu=[UDF objectForKey:@"houseDescribe5"];
    }else{
        miaoshu=@"";
    }
    if ([UDF objectForKey:@"phone5"]) {
        phone=[UDF objectForKey:@"phone5"];
    }else{
        phone=@"";
    }
    if ([UDF objectForKey:@"address5"]) {
        quyu=[UDF objectForKey:@"address5"];
    }else{
        quyu=@"";
    }
    if ([UDF objectForKey:@"neede5"]) {
        xinxilaiyuan=[UDF objectForKey:@"neede5"];
    }else{
        xinxilaiyuan=@"";
    }
    if ([UDF objectForKey:@"mianji5"]) {
        dec_mianji=[UDF objectForKey:@"mianji5"];
    }else{
        dec_mianji=@"";
    }

    NSArray *imageArr;
    if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage5.plist"]]) {
        imageArr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage5.plist"]];
    }
    NSData *data1;
    NSData *data2;
    NSData *data3;
    ...
    if (_judgeArr.count==0) {
        if ([CheckText nameWithObject:lianxiren]==YES&&[CheckText phoneWithObject:phone]==YES) {
            [AFHttpManager sendShangPuChuZuWithbiaoti:biaoti withdec_zujin:dec_zujin withdizhi:dizhi withdt_datetime:dt_datetime withdec_mianji:dec_mianji withgongxu:gongxu withlianxiren:lianxiren withmiaoshu:miaoshu withphone:phone withquyu:quyu withshangpuleixing:shangpuleixing withxinxilaiyuan:xinxilaiyuan withimg1:data1 withimg2:data2 withimg3:data3 withimg4:data4 withimg5:data5 withimg6:data6 withimg7:data7 withType:^(NSString *string) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *section2ListArr=@[@"供       需",@"租       金",@"面       积",@"商铺类型"];
    ...
    if (indexPath.section==0) {
        ...
        for (UIButton *button in _imageButtonArr) {
            if (button.tag==1) {
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage5.plist"]]) {
                    NSArray *arr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage5.plist"]];
        ...
        if (indexPath.row==0) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"address5"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"houseAddress5"];
        ...
        if (indexPath.row==0) {
            ...
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"gongxu5"] isEqualToString:@"求租"]) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"zujin5"];
            ...
            label.textAlignment=NSTextAlignmentRight;
            label.font=[UIFont systemFontOfSize:14.0];
            cell.detailTextField.text=[self writeWithKey:@"mianji5"];
            ...
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.text=[self writeWithKey:@"leixing5"];
        ...
        if (indexPath.row==0){
            cell.detailTextField.text=[self writeWithKey:@"houseTitle5"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"houseDescribe5"];
        ...
        if (indexPath.row==0) {
            cell.detailTextField.text=[self writeWithKey:@"person5"];
            ...
            cell.detailTextField.keyboardType=UIKeyboardTypeNumberPad;
            cell.detailTextField.text=[self writeWithKey:@"phone5"];
            ...
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"neede5"] isEqualToString:@"经纪人"]) {
                seg.selectedSegmentIndex=1;
            }else{
                seg.selectedSegmentIndex=0;
            }
...
-(void)changeGongxu:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"出租" forKey:@"gongxu5"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"求租" forKey:@"gongxu5"];
        }
            break;
        default:
            break;
...
-(void)changeNeede5:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"个人" forKey:@"neede5"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"经纪人" forKey:@"neede5"];
        }
            break;
        default:
            break;
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [_imageArr writeToFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage5.plist"] atomically:YES];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==1) {
        if (indexPath.row==0) {
            ...
            ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择区域" textArray:_addressArr withKey:@"address5"];
    ...
    if (indexPath.section==2&&indexPath.row==3){
        ...
        ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择区域" textArray:_leixingArr withKey:@"leixing5"];
...
-(void)chooseAddressClick:(UIButton *)button{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"address5"];
...
-(void)chooseLeixingClick:(UIButton *)button{
    ...
    ResumeTableViewCell *cell=(ResumeTableViewCell *)[_tableView cellForRowAtIndexPath:indexPath];
    cell.detailTextField.text=text;
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"leixing5"];
