...
@interface XieZiLouViewController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIScrollViewDelegate,UITextFieldDelegate>
{
    NSArray *_addressArr;
    NSArray *_leixingArr;
...
@implementation XieZiLouViewController
...
-(void)doBack:(id)sender{
    ...
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        ...
        [[NSFileManager defaultManager] removeItemAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/vhouseImage6.plist"] error:nil];
    }];
...
-(void)resumeButtonClick{
    ...
    if ([UDF objectForKey:@"houseTitle6"]) {
        biaoti=[UDF objectForKey:@"houseTitle6"];
    }else{
        biaoti=@"";
    }
    if ([UDF objectForKey:@"zujin6"]) {
        dec_zujin=[UDF objectForKey:@"zujin6"];
    }else{
        dec_zujin=@"";
    }
    if ([UDF objectForKey:@"houseAddress6"]) {
        dizhi=[UDF objectForKey:@"houseAddress6"];
    }else{
        dizhi=@"";
    }
    if ([UDF objectForKey:@"gongxu6"]) {
        gongxu=[UDF objectForKey:@"gongxu6"];
    }else{
        gongxu=@"";
    }
    if ([UDF objectForKey:@"leixing6"]) {
        leixing=[UDF objectForKey:@"leixing6"];
    }else{
        leixing=@"";
    }
    if ([UDF objectForKey:@"person6"]) {
        lianxiren=[UDF objectForKey:@"person6"];
    }else{
        lianxiren=@"";
    }
    if ([UDF objectForKey:@"houseDescribe6"]) {
        miaoshu=[UDF objectForKey:@"houseDescribe6"];
    }else{
        miaoshu=@"";
    }
    if ([UDF objectForKey:@"phone6"]) {
        phone=[UDF objectForKey:@"phone6"];
    }else{
        phone=@"";
    }
    if ([UDF objectForKey:@"address6"]) {
        quyu=[UDF objectForKey:@"address6"];
    }else{
        quyu=@"";
    }
    if ([UDF objectForKey:@"neede6"]) {
        xinxilaiyuan=[UDF objectForKey:@"neede6"];
    }else{
        xinxilaiyuan=@"";
    }
    if ([UDF objectForKey:@"mianji6"]) {
        dec_mianji=[UDF objectForKey:@"mianji6"];
    }else{
        dec_mianji=@"";
    }
    if ([UDF objectForKey:@"name6"]) {
        name=[UDF objectForKey:@"name6"];
    }else{
        name=@"";
    }

    NSArray *imageArr;
    if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage6.plist"]]) {
        imageArr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage6.plist"]];
    }
    NSData *data1;
    NSData *data2;
    NSData *data3;
    ...
    if (_judgeArr.count==0) {
        if ([CheckText nameWithObject:name]==YES&&[CheckText phoneWithObject:phone]==YES) {
            [AFHttpManager sendXieZiLouChuZuWithbiaoti:biaoti withbiaozhi:biaozhi withdec_zujin:dec_zujin withdizhi:dizhi withdt_datetime:dt_datetime withdec_mianji:dec_mianji withgongxu:gongxu withlianxiren:lianxiren withmiaoshu:miaoshu withphone:phone withquyu:quyu withleixing:leixing withxinxilaiyuan:xinxilaiyuan withname:name withimg1:data1 withimg2:data2 withimg3:data3 withimg4:data4 withimg5:data5 withimg6:data6 withimg7:data7 withType:^(NSString *string) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSArray *section2ListArr=@[@"供       需",@"售       价",@"名       称",@"面       积",@"类       型"];
    NSArray *section2PlaceholderArr=@[@"",@"数字填写，不能是0",@"2-10字，不能填写电话",@"数字填写，请输入",@"请选择"];
    NSArray *section3ListArr=@[@"标       题",@"描       述（选填）"];
    NSArray *section3PlaceholderArr=@[@"6-30字，不能填写电话",@"所在商圈等等"];
    ...
    if (indexPath.section==0) {
        ...
        for (UIButton *button in _imageButtonArr) {
            if (button.tag==1) {
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage6.plist"]]) {
                    NSArray *arr=[NSArray arrayWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage6.plist"]];
        ...
        if (indexPath.row==0) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"address6"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"houseAddress6"];
        ...
        if (indexPath.row==0) {
            UISegmentedControl *seg =[[UISegmentedControl alloc]initWithItems:@[@"出售",@"求购"]];
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"gongxu6"] isEqualToString:@"求购"]) {
            ...
            cell.detailTextField.text=[self writeWithKey:@"zujin6"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"name6"];
            [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(saveName5:) name:UITextFieldTextDidEndEditingNotification object:cell.detailTextField];
            cell.detailTextField.tag=10;
            ...
            label.textAlignment=NSTextAlignmentRight;
            label.font=[UIFont systemFontOfSize:14.0];
            cell.detailTextField.text=[self writeWithKey:@"mianji6"];
            ...
            cell.accessoryType=UITableViewCellAccessoryDisclosureIndicator;
            cell.detailTextField.text=[self writeWithKey:@"leixing6"];
        ...
        if (indexPath.row==0){
            cell.detailTextField.text=[self writeWithKey:@"houseTitle6"];
            ...
            cell.detailTextField.text=[self writeWithKey:@"houseDescribe6"];
        ...
        if (indexPath.row==0) {
            cell.detailTextField.text=[self writeWithKey:@"person6"];
            ...
            cell.detailTextField.keyboardType=UIKeyboardTypeNumberPad;
            cell.detailTextField.text=[self writeWithKey:@"phone6"];
            ...
            if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"neede6"] isEqualToString:@"经纪人"]) {
                seg.selectedSegmentIndex=1;
            }else{
                seg.selectedSegmentIndex=0;
            }
...
-(void)changeGongxu:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"出售" forKey:@"gongxu6"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"求购" forKey:@"gongxu6"];
        }
            break;
        default:
            break;
...
-(void)changeNeede5:(UISegmentedControl *)seg{
    switch (seg.selectedSegmentIndex) {
        case 0:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"个人" forKey:@"neede6"];
        }
            break;
        case 1:
        {
            [[NSUserDefaults standardUserDefaults]setObject:@"经纪人" forKey:@"neede6"];
        }
            break;
        default:
            break;
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    [_imageArr writeToFile:[NSHomeDirectory() stringByAppendingPathComponent:@"Documents/houseImage6.plist"] atomically:YES];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==1) {
        if (indexPath.row==0) {
            ...
            ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择区域" textArray:_addressArr withKey:@"address6"];
    ...
    if (indexPath.section==2&&indexPath.row==4){
        ...
        _leixingArr=@[@"写字楼",@"商务公寓",@"商务酒店",@"宾馆酒店",@"其他"];
        ChooseViewController *vc=[[ChooseViewController alloc]initWithTile:@"选择类型" textArray:_leixingArr withKey:@"leixing6"];
...
-(void)chooseAddressClick:(UIButton *)button{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"address6"];
...
-(void)chooseLeixingClick:(UIButton *)button{
    ...
    ResumeTableViewCell *cell=(ResumeTableViewCell *)[_tableView cellForRowAtIndexPath:indexPath];
    cell.detailTextField.text=text;
    [[NSUserDefaults standardUserDefaults]setObject:text  forKey:@"leixing6"];
