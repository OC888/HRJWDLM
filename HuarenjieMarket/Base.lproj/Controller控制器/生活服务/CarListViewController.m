...
@interface CarListViewController ()<UISearchResultsUpdating,UITableViewDataSource,UITableViewDelegate>
...
- (void)viewDidLoad {
    ...
    [AFHttpManager parserCarInfoWithArr:^(NSMutableArray *resumeArr) {
        [mb hide:YES];
...
-(void)writeRsume{
    ...
    ResumeCarViewController *resumeVC=[[ResumeCarViewController alloc]init];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CarListTableViewCell *cell = (CarListTableViewCell*)[tableView cellForRowAtIndexPath:indexPath];
    if (!cell)
    {
        cell= (CarListTableViewCell *)[[[NSBundle  mainBundle]  loadNibNamed:@"CarListTableViewCell" owner:self options:nil]  lastObject];
    }
    CarInfo *carInfo =_arr[indexPath.row];
    cell.titleLabel.text=carInfo.biaoti;
    cell.detailLabel.text=[NSString stringWithFormat:@"%@上牌 %@万公里",carInfo.shangpaidate,carInfo.lianxiren];
    cell.priceLabel.text=[NSString stringWithFormat:@"%@万元",carInfo.price];
    [cell.carImageView sd_setImageWithURL:[NSURL URLWithString:carInfo.image1] placeholderImage:[UIImage imageNamed:@"tu.png"]];
    cell.selectionStyle=UITableViewCellSelectionStyleNone;
    return cell;
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    CarDetailViewController *resumeVC=[[CarDetailViewController alloc]init];
    CarInfo *carInfo =_arr[indexPath.row];
    resumeVC.carInfo=carInfo;
    [self.navigationController pushViewController:resumeVC animated:YES];
